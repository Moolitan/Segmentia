...
KV_SAVE_DIR = ROOT / "results" / "05_context_segment_agent_kv" / "TTFT_test" / "kv_cache"

OUTPUT_PATH = ROOT / "results" / "05_context_segment_agent_kv" / "TTFT_test" / "ttft_results.csv"
...
_QUERY_TOK   = 1   # one dummy "query" token appended after the segment
...
def measure_ttft(
    ...
    """Return TTFT in seconds (time from first step() call to first output token)."""
    engine.add_request(
...
def offline_prefill_segment(
    ...
    measure_ttft(engine, req_id, segment_tokens, params)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="TTFT benchmark: baseline vs ContextSegmentKV (no HTTP)",
    ...
    if WARMUP_TOKENS > 0:
        ...
        measure_ttft(engine, next_id("warmup"), make_unique_prefix_tokens(WARMUP_TOKENS, seed=0),
    ...
    # Phase 2: TTFT measurement
    print("=" * 50)
    print("Phase 2: TTFT measurement")
    print("=" * 50)
    ...
    for seg_size in SEGMENT_SIZES:
        ...
        for prefix_size in PREFIX_SIZES:
            ...
            results.append(TTFTResult("baseline", prefix_size, seg_size, total, base_ms, 1.0))
            print(f"  baseline   prefix={prefix_size:5d}  seg={seg_size:5d}  "
                  f"total={total:6d}  TTFT={base_ms:7.1f} ms")
            ...
            if not args.offline_prefill_only:
                ...
                results.append(TTFTResult("kv_inject", prefix_size, seg_size, total, inject_ms, speedup))
                print(f"  kv_inject  prefix={prefix_size:5d}  seg={seg_size:5d}  "
                      f"total={total:6d}  TTFT={inject_ms:7.1f} ms  speedup={speedup:.2f}x")
    ...
    hdr = f"{'mode':10} {'prefix':>8} {'segment':>8} {'total':>7} {'TTFT(ms)':>10} {'speedup':>8}"
    print("\n" + hdr)
    print("-" * len(hdr))
    for r in results:
        spd = f"{r.speedup:.2f}x" if r.mode == "kv_inject" else "       -"
        print(f"{r.mode:10} {r.prefix_tokens:>8} {r.segment_tokens:>8} "
              f"{r.total_tokens:>7} {r.ttft_ms:>10.1f} {spd:>8}")
