"""Run paired SkillsBench quality trials for current and SDK 1.43.1 OpenHands."""
...
from common.driver import SystemVariant, make_server_config
...
SAMPLE_COLUMNS = (
    ...
    "vllm_restart_boundary",
    "container_restart_boundary",
    "wall_time_seconds",
    "agent_error",
    "verifier_error",
...
def _replace_base_image(dockerfile: str, image_id: str) -> str:
    lines = dockerfile.splitlines(keepends=True)
    for index, line in enumerate(lines):
        if line.lstrip().upper().startswith("FROM "):
            newline = "\n" if line.endswith("\n") else ""
            lines[index] = f"FROM {image_id}{newline}"
            return "".join(lines)
    raise RuntimeError("Task Dockerfile has no FROM instruction")
...
def _stage_workloads(
    ...
    staged_by_harness: dict[str, list[dict[str, Any]]] = {}
    for harness in local.HARNESSES:
        staged: list[dict[str, Any]] = []
        for workload in workloads:
            source = Path(workload["task_dir"])
            destination = (
                run_dir / "staged_tasks" / harness.harness_id / str(workload["task_id"])
            )
            if not destination.exists():
                shutil.copytree(source, destination)
            dockerfile = destination / "environment/Dockerfile"
            if harness.image_locked:
                original = (source / "environment/Dockerfile").read_text(
                    encoding="utf-8"
                )
                expected = _replace_base_image(original, str(image_lock["image_id"]))
                if dockerfile.read_text(encoding="utf-8") != expected:
            ...
            record["source_task_dir"] = source
            record["task_dir"] = destination
            staged.append(record)
        staged_by_harness[harness.harness_id] = staged
    return staged_by_harness
...
def _run_logged(
    ...
    *,
    ...
    if return_code != 0:
        raise RuntimeError(f"command exited with code {return_code}; see {log_path}")


def _bench_command(*arguments: str) -> list[str]:
    for path in (local.BENCH_PYTHON, local.BENCH_ENTRY):
        if not path.is_file():
            raise FileNotFoundError(f"required BenchFlow entry does not exist: {path}")
    return [str(local.BENCH_PYTHON), str(local.BENCH_ENTRY), *arguments]
...
def _required_skill_invocations(
    ...
    for line in trajectory.read_text(encoding="utf-8").splitlines():
        ...
        if event.get("type") != "tool_call" or event.get("kind") != "skill":
            continue
        serialized = json.dumps(event, ensure_ascii=False, sort_keys=True)
        if any(skill_name in serialized for skill_name in required):
            count += 1
    return count
...
def _vllm_totals(trace_path: Path, log_path: Path) -> dict[str, int | str]:
    ...
    if request_count == 0 and log_path.is_file():
        marker = 'POST /v1/chat/completions HTTP/1.1" 200 OK'
        request_count = sum(
            marker in line
...
def _sample_is_healthy(sample: Mapping[str, Any]) -> bool:
    """Pipeline health excludes task/Skill quality outcomes from its gate."""
    return all(
        (
...
def _prepare_docker_config(run_dir: Path) -> Path:
    ...
    if not plugin.is_file() or not os.access(plugin, os.X_OK):
        raise FileNotFoundError(f"Docker Compose plugin is not executable: {plugin}")
    ...
    proxy_config = {
        ...
        "httpsProxy": os.getenv("https_proxy") or os.getenv("HTTPS_PROXY") or "",
        "noProxy": os.getenv("no_proxy") or os.getenv("NO_PROXY") or "",
    }
    if not proxy_config["httpProxy"] or not proxy_config["httpsProxy"]:
        raise RuntimeError("host-network Docker setup requires HTTP/HTTPS proxy")
...
def _validate_image_lock() -> tuple[dict[str, Any], dict[str, Any]]:
    if not local.IMAGE_LOCK_FILE.is_file():
        raise FileNotFoundError(f"Missing immutable image lock: {local.IMAGE_LOCK_FILE}")
    lock = json.loads(local.IMAGE_LOCK_FILE.read_text(encoding="utf-8"))
    if lock.get("source_sha256") != source_sha256():
        raise RuntimeError("Adapter source differs from image.lock.json")
    inspected = inspect_image(local.LATEST_IMAGE_TAG)
    if inspected["image_id"] != lock.get("image_id"):
        raise RuntimeError(
            f"Image tag drift: {inspected['image_id']} != {lock.get('image_id')}"
        )
    labels = inspected["labels"]
    expected_labels = {
        "io.cskcache.openhands-sdk.version": local.OPENHANDS_SDK_VERSION,
        "io.cskcache.openhands-tools.version": local.OPENHANDS_TOOLS_VERSION,
        "io.cskcache.adapter.source-sha256": lock["source_sha256"],
    }
    for key, value in expected_labels.items():
        if labels.get(key) != value:
            raise RuntimeError(f"Image label mismatch for {key}: {labels.get(key)!r}")
...
def _run_smoke_gates(
    ...
    root = run_dir / "smoke"
    marker = root / "smoke_passed.json"
    if marker.is_file():
        return
    attempt = _next_attempt(root)
    workspace = attempt / "workspace"
    workspace.mkdir()
    skills_dir = Path(workload["source_task_dir"]) / "environment/skills"
    required_skill = str(workload["required_skills"][0])
    ...
    common = [
        ...
        uid_gid,
        "-v",
        f"{skills_dir}:/skills:ro",
        "-v",
        f"{workspace}:/workspace",
        "-w",
        "/workspace",
        str(image_lock["image_id"]),
    ]
    runtime_log = attempt / "runtime_smoke.log"
    _run_logged(
        [
            *common,
            "python",
            "/opt/cskcache/openhands_sdk_1_43_1/smoke_runtime.py",
            "--skills-dir",
            "/skills",
            "--skill",
            required_skill,
    ...
    verifier_log = attempt / "verifier_smoke.log"
    _run_logged(
        [
            *common,
            "python",
            "/opt/cskcache/openhands_sdk_1_43_1/verify_smoke.py",
            "/workspace/terminal_smoke_artifact.json",
        ],
        log_path=verifier_log,
        timeout_seconds=60,
    )
    runtime = _last_json_line(runtime_log)
    verifier = _last_json_line(verifier_log)
    gates = {
        "version_and_image_fingerprint": (
            runtime.get("versions")
            == {
                "openhands-sdk": local.OPENHANDS_SDK_VERSION,
                "openhands-tools": local.OPENHANDS_TOOLS_VERSION,
            }
            and inspected.get("image_id") == image_lock.get("image_id")
        ),
        "invoke_skill_full_body": runtime.get("invoke_exact_full_body") is True,
        "terminal_artifact_verifier_readable": verifier.get("verifier_readable")
        is True,
    }
    report = {
        "gates": gates,
        "passed": all(gates.values()),
        "image_lock": dict(image_lock),
        "image_inspect": dict(inspected),
        "runtime": runtime,
        "verifier": verifier,
        "attempt_dir": str(attempt),
        "completed_utc": utc_now(),
    }
    (attempt / "smoke_report.json").write_text(
        json.dumps(report, indent=2, ensure_ascii=False, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    if not report["passed"]:
        raise RuntimeError(f"Smoke gates failed: {attempt / 'smoke_report.json'}")
    marker.write_text(
        json.dumps(report, indent=2, ensure_ascii=False, sort_keys=True) + "\n",
        encoding="utf-8",
    )
...
def _run_case(
    *,
    ...
    task_id = str(workload["task_id"])
    task_dir = Path(workload["task_dir"])
    case_id = f"{platform_id}__{harness.harness_id}__{task_id}__r{repetition}"
    ...
    try:
        ...
        sample = {
            ...
            "agent": harness.agent,
            "sdk_version": harness.sdk_version,
            "tools_version": harness.tools_version,
            "image_id": image_lock["image_id"] if harness.image_locked else "",
            "task_id": task_id,
            ...
            "required_skill_used": required_skill_invocations > 0,
            ...
            **vllm,
            "vllm_restart_boundary": case_id,
            "container_restart_boundary": case_id,
        ...
        if not healthy:
            raise RuntimeError(
                "BenchFlow completed but failed the pipeline-health gate; "
                f"see {attempt / 'parsed_result.json'}"
            )
        append_jsonl(run.samples_jsonl, sample)
        append_csv(run.samples_csv, sample, SAMPLE_COLUMNS)
        ...
        print(f"[case-failed] {case_id}: {failure['error']}", flush=True)
        if not local.CONTINUE_AFTER_CASE_FAILURE:
            raise
        return False
...
def main() -> None:
    ...
    image_lock, image_inspect = _validate_image_lock()
    values = {
        ...
        "harnesses": [harness.__dict__ for harness in local.HARNESSES],
        "latest_image": image_lock,
        ...
        "restart_boundary": "(harness, task, repetition)",
        "case_order": "repetition -> workload -> harness",
    }
    config_paths = (
        Path(__file__),
        Path(local.__file__),
    ...
    staged = _stage_workloads(run.run_dir, workloads, image_lock)
    smoke_input = dict(workloads[0])
    smoke_input["source_task_dir"] = Path(workloads[0]["task_dir"])
    _run_smoke_gates(run.run_dir, smoke_input, image_lock, image_inspect)
    if local.RUN_ORACLE_PREFLIGHT:
        for workload in staged["current"]:
    ...
    for platform_index, platform_id in enumerate(local.PLATFORM_IDS):
        ...
        for repetition in range(local.REPETITIONS):
            for workload_index in range(len(workloads)):
                for harness in local.HARNESSES:
                    workload = staged[harness.harness_id][workload_index]
                    passed = _run_case(
                        run=run,
                        platform_id=platform_id,
                    ...
                    if not passed:
                        failures.append(
                            f"{platform_id}/{harness.harness_id}/"
                            f"{workload['task_id']}/r{repetition}"
                        )

    from analyze import analyze

    if run.samples_csv.is_file():
        analyze(run.run_dir)
    (run.run_dir / "completion.json").write_text(
        json.dumps(
            {
                "failures": failures,
                "all_cases_healthy": not failures,
                "completed_utc": utc_now(),
            },
            indent=2,
            ensure_ascii=False,
    ...
    if failures:
        raise RuntimeError(
            f"{len(failures)} case(s) failed; attempts were preserved below {run.run_dir}"
        )
    run.finish()
