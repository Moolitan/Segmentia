#!/usr/bin/env bash
# P0 logprob / margin diagnostic.
#
# This wrapper restarts vLLM per (task, mode), then calls
# run_margin_diagnostic.py with logprobs enabled. It is meant to be run by the
# user, not automatically by the coding agent.
#
# Usage:
#   bash scripts/06_context_free_segment_cache/run_margin_diagnostic.sh
#   TASKS=doc_coauthoring_design_doc,slack_launch_pack bash scripts/06_context_free_segment_cache/run_margin_diagnostic.sh
#   MODES=recompute,rope MAX_TOKENS=256 TOP_LOGPROBS=10 bash scripts/06_context_free_segment_cache/run_margin_diagnostic.sh
set -euo pipefail
...
OUT_DIR="$RES/logprob_margin_diagnostic"
...
DECISION_WINDOW="${DECISION_WINDOW:-128}"
KV_DIR="${KV_DIR:-$SEGMENTIA_OUTPUT_DIR/offline_skill_kv}"
HEADLINE_JSONL="${HEADLINE_JSONL:-$RES/headline_semantic_action_gap/data/decode_outputs.jsonl}"
TOKEN_JSONL="${TOKEN_JSONL:-$OUT_DIR/data/logprob_margin_rows.jsonl}"
CASE_CSV="${CASE_CSV:-$OUT_DIR/tables/margin_case_summary.csv}"
...
for task in "${TASK_LIST[@]}"; do
  ...
  for mode in "${MODE_LIST[@]}"; do
    ...
    echo "--- margin diagnostic mode=$mode task=$task ---"
    python "$SCRIPT_DIR/run_margin_diagnostic.py" \
      --tasks "$task" \
      --modes "$mode" \
