...
运行:
  ...
  python scripts/04_kv_cache_research/kvreused_naturalreferenceblock/run_kv_reuse3.py
...
OUT_DIR    = RESULTS_ROOT / "kv_reuse_natural_reference_block" / "no_query_v2"
FIG_DIR    = OUT_DIR / "figures"
OUT_DIR.mkdir(parents=True, exist_ok=True)
FIG_DIR.mkdir(parents=True, exist_ok=True)

MODEL_PATH = "/mnt/Large_Language_Model_Lab_1/llm_models/Qwen3-14B/Qwen/Qwen3-14B"
SCENARIO   = "KVReuse_NaturalReferenceBlock_NoQuery_v2"
...
[user]
One more constraint before we start: these updates are for an internal platform review. Please keep
the tone plain and operational, avoid hype, and stay close to the facts I provide.

[assistant]
Understood. I will keep the wording direct, specific, and operational.

[user]
Using that format, write an update about the dataset sync migration.

Context:
- This is for the weekly platform review, so the update should help leads quickly assess rollout readiness.
- The migration goal is to replace the legacy sync path before month-end without surprising downstream analytics users.
...
[user]
This is the right shape. Keep the same level of specificity for the next one, and do not add any
extra intro line before the title. I want the next response to begin directly with the update title.

[assistant]
Understood. I will continue with the same structure and start directly with the title.
...
- Context: this feature matters because researchers currently need manual help from the platform team after failed long-running tasks.
...
assert skill2_end < N_full, "run_kv_reuse3.py 需要在 skill_v2 后保留 assistant 续写前缀"
...
assert query_ids, "run_kv_reuse3.py 需要 assistant 续写前缀"
