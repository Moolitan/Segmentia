...
def raw_backend_extra_config() -> dict[str, object]:
    return {
...
def lmcache_extra_config() -> dict[str, object]:
    """Configuration for the dedicated offline exact-save service."""

    return {
        "exact_save_kv_2td": True,
        **raw_backend_extra_config(),
    }


def serving_lmcache_extra_config() -> dict[str, object]:
    """Configuration for online T0 prefetch from this Catalog/container."""

    return {
        "csk_t0_prefetch": True,
        "cskcache_metadata_path": str(CATALOG),
        "csk_raw_backend_name": "raw_block",
        "csk_prefetch_handle_ttl_seconds": 60,
        **raw_backend_extra_config(),
    }
...
def main() -> None:
    ...
    if args.command == "initialize":
        ...
        initialize()
    elif args.command in {"lmcache-config", "serve-lmcache-config"}:
        if args.quarantine_unrecoverable:
            raise SystemExit(
                "--quarantine-unrecoverable is only valid with finalize"
            )
        config = (
            ...
            if args.command == "lmcache-config"
            else serving_lmcache_extra_config()
        )
