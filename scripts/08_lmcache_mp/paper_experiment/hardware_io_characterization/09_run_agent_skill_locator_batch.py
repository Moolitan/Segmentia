...
def validate_locator_manifest(cache_info: dict[str, Any]) -> None:
    ...
    if manifest.get("schema_version") != 4 or not isinstance(locator, dict):
        raise ValueError(
            f"direct-reuse locator experiment requires schema-v4 cache: "
            f"{manifest_path}"
        )
    if locator.get("kind") != "context_segment_start_marker_v1":
