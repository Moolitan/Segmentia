#!/usr/bin/env bash
# Real GPU closure: source SSD -> restart -> Bank owner -> READY follower.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CAPTURE_DIR="$ROOT/scripts/08_lmcache_mp/cross_request_kv_capture"
...
OUTPUT_ROOT="${SEGMENTIA_SHARED_GPU_OUTPUT_ROOT:-/mnt/Large_Language_Model_Lab_1/wsh/Segmentia/output/08_lmcache_mp/shared_bank_gpu_closure_runs}"
RUN_DIR="$OUTPUT_ROOT/$RUN_ID"
RESULT_DIR="${SEGMENTIA_SHARED_GPU_RESULT_DIR:-$ROOT/results/problem_exploration/shared_skill_gpu_closure}"
SEPARATOR="${LMCACHE_BLEND_SPECIAL_STR:-<|fim_pad|><|repo_name|>}"
PYTHONPATH_VALUE="$SCRIPT_DIR:$CAPTURE_DIR:$ROOT/vllm:$ROOT/LMCache${PYTHONPATH:+:$PYTHONPATH}"
...
# Phase 2's shared Attention metadata and per-layer mu workspace are currently
# implemented in vLLM's V1 GPUModelRunner. Fail closed instead of silently
# selecting the default V2 runner, which has not been wired yet.
...
if envs.VLLM_USE_V2_MODEL_RUNNER is not False:
    raise RuntimeError("shared GPU closure requires the V1 Model Runner")
...
send_request() {
  local role="$1"
  local output="$2"
  PYTHONPATH="$PYTHONPATH_VALUE" "$PYTHON_BIN" "$SCRIPT_DIR/send_request.py" \
    --spec "$RUN_DIR/requests/$role.json" \
    --output "$output" \
...
echo "[phase 0/3] prepare frozen real Skill and controlled follower context"
...
send_request source "$RUN_DIR/source/request.json"
...
echo "[phase 2/3] restart -> rehydrate -> owner loads and publishes READY"
start_server target "$RUN_DIR/shared_ssd" "$RUN_DIR/target/vllm.log"
send_request owner "$RUN_DIR/target/owner.json"
PYTHONPATH="$PYTHONPATH_VALUE" "$PYTHON_BIN" "$SCRIPT_DIR/wait_for_event.py" \
  --log "$RUN_DIR/target/vllm.log" \
  --event segmentia_shared_bank_publish \
  --response-record "$RUN_DIR/target/owner.json"

echo "[phase 3/3] READY follower -> calibration-only correction -> shared attention"
send_request follower "$RUN_DIR/target/follower.json"
cleanup_server
