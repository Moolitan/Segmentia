...
sys.path.insert(0, os.path.join(ROOT, "scripts", "03_8B"))
...
def main():
    ...
    parser.add_argument("--workspace", default=os.path.join(ROOT, "workspace", "03_8B"),
                        help="任务工作目录")
    parser.add_argument("--skills-dir", default=None,
                        help="Skills 源目录(默认: <workspace>/.agents/skills,若不存在则用仓库 skills/)")
    parser.add_argument("--output", default=os.path.join(ROOT, "results", "03_8B", "multurn_bench",
                                                          "multiturn_sequence_traces.json"),
                        help="结果 JSON 输出路径(bench.sh 传入 $seq_res/multiturn_sequence_traces.json)")
    parser.add_argument("--vllm-port", type=int, default=8000)
    parser.add_argument("--log-dir", default=os.path.join(ROOT, "log", "03_8B"),
                        help="日志目录")
