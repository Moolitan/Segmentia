...
run_kv_reuse.py
================
KV 复用实验:在第二次重复参考文本之后接一段 QUERY_TEXT,在 query 末尾观测 KV 偏差对 decode 的影响。
...
序列骨架:
  [USER_CONTEXT] + [SKILL_v1] + [MIDDLE_HISTORY] + [SKILL_v2] + [QUERY]

场景:
  A  (参考)      :正常 prefill skill_v2 + query,拿 query 末尾 logit(ground truth)
  B1 (朴素复用)  :复用 skill_v1 全部 L 个 KV(位置错误),再正常 prefill query
  B2 (RoPE 校正) :V 照搬,K 逆旋转再重旋转到 skill_v2 位置(全 L 个),再正常 prefill query
...
运行:
  ...
  python scripts/04_kv_cache_research/kvreused_naturalreferenceblock/run_kv_reuse.py
...
OUT_DIR    = RESULTS_ROOT / "kv_reuse_natural_reference_block" / "with_query"
FIG_DIR    = OUT_DIR / "figures"
OUT_DIR.mkdir(parents=True, exist_ok=True)
FIG_DIR.mkdir(parents=True, exist_ok=True)

MODEL_PATH = "/mnt/Large_Language_Model_Lab_1/llm_models/Qwen3-14B/Qwen/Qwen3-14B"
SCENARIO   = "KVReuse_NaturalReferenceBlock"
...
#   3) USER_CONTEXT / MIDDLE_HISTORY / QUERY_TEXT 之间要有自然语言层面的逻辑连续性
#
# 序列结构:
#   USER_CONTEXT  : 用户先贴一份"平台更新格式参考卡"
#   SKILL_TEXT    : 参考卡全文(精确出现两次)
#   MIDDLE_HISTORY: 模型先根据该参考卡完成一个数据迁移更新,然后用户说要再贴一次同样参考卡
#   SKILL_TEXT    : (第二次,相同内容)
#   QUERY_TEXT    : 第二个真实任务请求 + "[assistant]\n\n"
#   → decode 从 query 末尾开始,模型即将生成新 update 的第一个 token
# ============================================================
...
# USER_CONTEXT: 用户先要求"读一下参考卡,留待后续写作"
USER_CONTEXT = """\
[system]
...
# MIDDLE_HISTORY: 先完成一个数据迁移更新,然后用户说下面会再贴一次同样的参考卡
MIDDLE_HISTORY = """
[assistant]
...
[user]
That format works. I have another update request, and I am pasting the same reference card again
below so we use the exact same format.
...
assert skill2_end < N_full, "FULL_PROMPT 末尾应有 QUERY_TEXT token"
...
# query_ids: QUERY_TEXT tokens(紧接在 skill_v2 之后)
...
# ============================================================
# 8. Phase 3A:Scenario A(正常 prefill skill_v2 + query)
# ============================================================
print("\n" + "=" * 60)
print(f"Phase 3A: Scenario A -- prefill [skill_v2]({len(skill2_ids)} tok) + [query]({len(query_ids)} tok)")
t0 = time.time()
...
# ============================================================
# 直接复用 skill_v1 全部 L 个 KV(位置错误),再正常 prefill query。
print("\n" + "=" * 60)
print(f"Phase 3B1: Scenario B1 -- naive reuse (all {L_skill} KV), prefill query")
...
# ============================================================
print("\n" + "=" * 60)
print(f"Phase 3B2: Scenario B2 -- RoPE-corrected reuse (all {L_skill} KV), prefill query")
...
assert cache_B2_pre.get_seq_length() == skill2_end
...
# step 0:来自 prefill 最后一个 token 的 logits
...
results = {
    ...
    "sequence_info": {
        ...
        "note":               "decode starts after skill_v2 + query; logits measured at query end",
    },
    "y_A":      y_A,
