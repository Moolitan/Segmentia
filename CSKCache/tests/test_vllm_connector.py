...
def test_non_aligned_reuse_boundary_uses_exact_slots_and_covering_blocks() -> None:
    connector = make_connector()
    plan = ReusePlan(
        ticket="call-nonaligned",
        cache_object_id="skill-v1",
        request_id="request-nonaligned",
        segment_start=100,
        segment_end=109,
        reuse_start=103,
        reuse_end=109,
        source_reuse_start=3,
        source_reuse_end=9,
        calibration_start=100,
        calibration_end=103,
        correction_alpha=0.6,
    ...
    connector._pending_worker_requests = {}
    request = SimpleNamespace(
        request_id=plan.request_id, all_token_ids=list(range(120))
    )

    connector.update_state_after_alloc(request, SimpleNamespace(), 9)

    worker = connector._pending_worker_requests[plan.request_id]
    assert worker.plan.reuse_start == 103
    assert worker.plan.reuse_end == 109
    assert worker.slot_mapping.tolist() == list(range(109))
    assert worker.failed_block_ids == frozenset({25, 26, 27})
...
def test_readiness_uses_existing_control() -> None:
    connector = make_connector()
    lookup = connector._lmcache_engine.lookup_client
...
def test_rank_readiness_waits_for_every_worker_then_returns_fixed_plan() -> None:
    connector = make_connector()
    plan = make_plan()
    connector._csk_rank_control = MagicMock()
    connector._csk_rank_control.execute_all.return_value = [
        {"status": "ready", "plan": plan.to_dict(), "reason": None},
        {"status": "loading", "plan": plan.to_dict(), "reason": None},
    ]
