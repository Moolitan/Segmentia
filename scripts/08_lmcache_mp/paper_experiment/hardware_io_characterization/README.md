...
Tests 01--06 and 08 do not start vLLM. Tests 07, 09, and 10 are long-running Agent
experiments that start one independent vLLM server per task. None of these
tests modifies the offline KV tensors.
...
选择11个Skill，复制每层`.pt`、sidecar和完成标记，完整校验SHA-256，然后只重写
...
/mnt/990_pro/skill_save_pool/cskcache_raw/qwen3_14b_skill_kv_v1_manifest.json
```
...
复用`common.py`中的底层文件/配置工具，并在自身代码中完成schema-v4 cache校验、
