...
#
# Runs independent vLLM lifecycles per task. For each task, first dump
# full-context recompute KV, then restart vLLM and run reuse where source KV is
# collected only within that reuse pass. Finally computes CKSim across tasks.
#
# Usage:
#   bash run_cksim.sh
...
start_vllm_for_cksim() {
  ...
  export VLLM_CONTEXT_SEGMENT_KV_DIR="$KV_SAVE_DIR"
